...
-- (Versão segura - só cria o que não existe)
-- =====================================================
...
DROP POLICY IF EXISTS "Users view their organizations" ON public.organizations;
...
DROP POLICY IF EXISTS "Users view modules in their orgs" ON public.organization_modules;
...
DROP POLICY IF EXISTS "Users access branches in their org" ON public.branches;
...
DROP POLICY IF EXISTS "Users access clients in their org with CRM" ON public.clients;
...
DROP POLICY IF EXISTS "Users access products in their org" ON public.products;
...
DROP POLICY IF EXISTS "Users access invoices in their org" ON public.invoices;
...
DROP POLICY IF EXISTS "Users access invoice items via invoice" ON public.invoice_items;
...
DROP POLICY IF EXISTS "Users access payments in their org" ON public.payments;
...
DROP POLICY IF EXISTS "Users access agt_submissions in their org" ON public.agt_submissions;
...
DROP POLICY IF EXISTS "Users access categories in their org" ON public.product_categories;
...
DROP TRIGGER IF EXISTS set_updated_at_agt_submissions ON public.agt_submissions;
...
INSERT INTO public.subscription_plans (name, description, price_monthly, price_yearly, max_users, max_branches, max_invoices_per_month, features) 
SELECT * FROM (VALUES
  ('Starter', 'Ideal para pequenos negócios começando', 0::numeric, 0::numeric, 1, 1, 50, 
   '{"modules": {"documents": true, "crm": false, "products": false, "payments": false, "inventory": false, "reports": false, "api": false, "pos": false}, "limits": {"storage_gb": 1}}'::jsonb),
  ('Business', 'Para empresas em crescimento', 15000::numeric, 150000::numeric, 3, 2, 500,
   '{"modules": {"documents": true, "crm": true, "products": true, "payments": false, "inventory": false, "reports": true, "api": false, "pos": false}, "limits": {"storage_gb": 10}}'::jsonb),
  ('Pro', 'Solução completa para empresas estabelecidas', 35000::numeric, 350000::numeric, 10, 5, 2000,
   '{"modules": {"documents": true, "crm": true, "products": true, "payments": true, "inventory": true, "reports": true, "api": false, "pos": true}, "limits": {"storage_gb": 50}}'::jsonb),
  ('Enterprise', 'Solução customizada sem limites', 0::numeric, 0::numeric, 999, 999, 999999,
   '{"modules": {"documents": true, "crm": true, "products": true, "payments": true, "inventory": true, "reports": true, "api": true, "pos": true}, "limits": {"storage_gb": 500}}'::jsonb)
) AS v(name, description, price_monthly, price_yearly, max_users, max_branches, max_invoices_per_month, features)
...
CREATE INDEX IF NOT EXISTS idx_organization_modules_enabled ON public.organization_modules(organization_id, is_enabled);
...
CREATE INDEX IF NOT EXISTS idx_agt_submissions_org_id ON public.agt_submissions(organization_id);
CREATE INDEX IF NOT EXISTS idx_agt_submissions_invoice_id ON public.agt_submissions(invoice_id);